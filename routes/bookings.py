...
def get_session_and_member(session_id: str, member_id: str):
    ...
    return session.data[0] if session.data else None, member.data[0] if member.data else None

@router.get("/session/{session_id}")
def get_session_bookings(session_id: str):
...
def create_booking(booking: BookingCreate):
    ...
    if existing.data:
        raise HTTPException(status_code=400, detail="Member already booked for this session")
